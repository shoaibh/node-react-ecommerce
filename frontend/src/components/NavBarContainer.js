...
const arrowStyleSubmenu = (subMenuCategorySelected, id, arrowDown) => subMenuCategorySelected === id && <div style={arrowDown}></div>
...
class NavbarContainer extends Component {
  ...
  render() {
    ...
    isBrowser ?            
      (<NavItem style={itemMenu} onMouseEnter={()=>this.handleSubMenuEnter(id)} >
        <NavLink to={`/category/${id}`}  className="text-white" >{id.charAt(0).toUpperCase() + id.slice(1)}</NavLink> {arrowStyleSubmenu(subMenuCategorySelected, id, arrowDown)}
      </NavItem>) :
      (<NavItem style={itemMenu}>
      <NavLink to={`/category/${id}`}  className="text-white" >{id}</NavLink>
    </NavItem>)
    ...
    return (
      <div className="sticky-top">
        <Navbar light expand="md" style={navbarBackground}>
          ...
          <NavItem style={itemMenu}><NavLink to='/cart'  className="text-white mr-5">cart</NavLink>
          {this.props.userInfo ? (
              <NavLink to="/profile" className="text-white mr-5">{this.props.userInfo.name}</NavLink>
            ) : (
              <NavLink to="/signin" className="text-white mr-5">Sign In</NavLink>
            )}
            {this.props.userInfo && this.props.userInfo.isAdmin && (
              <div className="dropdown">
                <a href="#" className="text-white mr-5">Admin</a>
                <ul className="dropdown-content">
                  <li>
                    <Link to="/orders"  className="text-white mr-3">Orders</Link>
                    <Link to="/products"  className="text-white mr-3">Products</Link>
                  </li>
                </ul>
              </div>
            )}</NavItem>
